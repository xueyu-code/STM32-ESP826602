...
static int8_t ICACHE_FLASH_ATTR gizCheckReport(dataPoint_t *cur, dataPoint_t *last)
{
    ...
    if(last->valueMQ9_data != cur->valueMQ9_data)
    {
        if(currentTime - lastReportTime >= REPORT_TIME_MAX)
        {
            GIZWITS_LOG("valueMQ9_data Changed\n");
            ret = 1;
        }
    }
    if(last->valueMQ5_data != cur->valueMQ5_data)
    {
        if(currentTime - lastReportTime >= REPORT_TIME_MAX)
        {
            GIZWITS_LOG("valueMQ5_data Changed\n");
            ret = 1;
        }
    }
...
static int8_t ICACHE_FLASH_ATTR gizDataPoints2ReportData(dataPoint_t *dataPoints , devStatus_t *devStatusPtr)
{
    ...
    devStatusPtr->valueMQ9_data = exchangeBytes(gizY2X(MQ9_data_RATIO,  MQ9_data_ADDITION, dataPoints->valueMQ9_data)); 
    devStatusPtr->valueMQ5_data = exchangeBytes(gizY2X(MQ5_data_RATIO,  MQ5_data_ADDITION, dataPoints->valueMQ5_data)); 
